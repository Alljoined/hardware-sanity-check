def epoching(args, seed):
    ...
    for s in range(args.n_ses):
        ...
        # Create a dictionary mapping each event type to a single integer
        event_dict = {key: value for key, value in event_id.items()}

        # Epoch into events and drop channels
        epochs = mne.Epochs(raw, events, event_id=event_dict, tmin=-0.05, tmax=0.60, baseline=(None,0), preload=True)

        # Check if 'Status' channel exists and drop it if it does
        if 'Status' in epochs.ch_names:
